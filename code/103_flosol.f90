...
! --- Smooth the velocity inside immersed boundary if IBM is applied
if (flag_ibm) then
    call smoothVel_cubic(u(1:nxt, 1:nynpy, 0:nz), 'x', phi_uv)
    call smoothVel_cubic(v(1:nxt, 1:nynpy, 0:nz), 'x', phi_uv)
    call smoothVel_cubic(w(1:nxt, 1:nynpy, 0:nz), 'x', phi_w)
end if
...
! --- Smooth the velocity inside immersed boundary if IBM is applied
if (flag_ibm) then
    call smoothVel_cubic(u(1:nxt, 1:nynpy, 0:nz), 'y', phi_uv)
    call smoothVel_cubic(v(1:nxt, 1:nynpy, 0:nz), 'y', phi_uv)
    call smoothVel_cubic(w(1:nxt, 1:nynpy, 0:nz), 'y', phi_w)
end if
...
! --- Restore the velocity field if IBM is applied
if (flag_ibm) then
    call restoreval_vel(u(1:nxt, 1:nynpy, 0:nz), phi_uv)
    call restoreval_vel(v(1:nxt, 1:nynpy, 0:nz), phi_uv)
    call restoreval_vel(w(1:nxt, 1:nynpy, 0:nz), phi_w)
end if
...
call mpi_sendrecv(txz(:, :, nz - 1), nxt*nynpy, MPI_RPREC, up,   4, &
                  txz(:, :, 0),      nxt*nynpy, MPI_RPREC, down, 4, &
                  comm, status, ierr)
call mpi_sendrecv(tyz(:, :, nz - 1), nxt*nynpy, MPI_RPREC, up,   5, &
                  tyz(:, :, 0),      nxt*nynpy, MPI_RPREC, down, 5, &
                  comm, status, ierr)
call mpi_sendrecv(tzz(:, :, nz - 1), nxt*nynpy, MPI_RPREC, up,   6, &
                  tzz(:, :, 0),      nxt*nynpy, MPI_RPREC, down, 6, &
                  comm, status, ierr)
call mpi_sendrecv(tzz(:, :, 1),      nxt*nynpy, MPI_RPREC, down, 7, &
                  tzz(:, :, nz),     nxt*nynpy, MPI_RPREC, up,   7, &
                  comm, status, ierr)
...
if (flag_ibm) then 
    call smoothTau_cubic(txx(1:nxt, 1:nynpy, 0:nz), 'x', phi_uv)
    call smoothTau_cubic(txy(1:nxt, 1:nynpy, 0:nz), 'y', phi_uv)
end if 
call divstress_uv(divtx, txx, txy, txz)

if (flag_ibm) then
    call smoothTau_cubic(txy(1:nxt, 1:nynpy, 0:nz), 'x', phi_uv)
    call smoothTau_cubic(tyy(1:nxt, 1:nynpy, 0:nz), 'y', phi_uv)
end if
call divstress_uv(divty, txy, tyy, tyz)

if (flag_ibm) then
    call smoothTau_cubic(txz(1:nxt, 1:nynpy, 0:nz), 'x', phi_txz)
    call smoothTau_cubic(tyz(1:nxt, 1:nynpy, 0:nz), 'y', phi_txz)
  end if
call divstress_w(divtz, txz, tyz, tzz)  ! --- provides divtz 1:nz-1, except 1:nz at top process 


if (flag_ibm) then
    call restoreval_tau(txx(1:nxt, 1:nynpy, 0:nz), phi_uv)
    call restoreval_tau(txy(1:nxt, 1:nynpy, 0:nz), phi_uv)
    call restoreval_tau(tyy(1:nxt, 1:nynpy, 0:nz), phi_uv)
    call restoreval_tau(tzz(1:nxt, 1:nynpy, 0:nz), phi_uv)
    call restoreval_tau(txz(1:nxt, 1:nynpy, 0:nz), phi_txz)
    call restoreval_tau(tyz(1:nxt, 1:nynpy, 0:nz), phi_txz)
  end if

! --- provides RHS{x,y,z} 1:nz-1
call convec(RHSx, RHSy, RHSz)

call mpi_sendrecv(divtx(:, :, nz - 1), nxt*nynpy, MPI_RPREC, up,   6, &
                  divtx(:, :, 0),      nxt*nynpy, MPI_RPREC, down, 6, &
                  comm, status, ierr)
call mpi_sendrecv(divtx(:, :, 1),      nxt*nynpy, MPI_RPREC, down, 7, &
                  divtx(:, :, nz),     nxt*nynpy, MPI_RPREC, up,   7, &
                  comm, status, ierr)
! ---
...
call mpi_sendrecv(p(1, 1, nz - 1), nxt*nynpy, MPI_RPREC, up,   2, &
                  p(1, 1, 0),      nxt*nynpy, MPI_RPREC, down, 2, &
                  comm, status, ierr)
call mpi_sendrecv(p(1, 1, 1),  nxt*nynpy, MPI_RPREC, down, 3, &
                  p(1, 1, nz), nxt*nynpy, MPI_RPREC, up,   3, &
                  comm, status, ierr)
!--calculate dpdz here
...
call mpi_sendrecv(ke(1, 1, nz-1), nxt*nynpy, MPI_RPREC, up,   1,   &
                  ke(1, 1, 0)   , nxt*nynpy, MPI_RPREC, down, 1,   &
                  comm, status, ierr)
call mpi_sendrecv(ke(1, 1, 1) , nxt*nynpy, MPI_RPREC, down, 2,     &
                  ke(1, 1, nz), nxt*nynpy, MPI_RPREC, up,   2,     &
                  comm, status, ierr)
ke_temp = ke
...
call mpi_sendrecv(dudt(1, 1, nz - 1), nxt*nynpy, MPI_RPREC, up,   1,   &
                  dudt(1, 1, 0),      nxt*nynpy, MPI_RPREC, down, 1,   &
                  comm, status, ierr)

call mpi_sendrecv(dvdt(1, 1, nz - 1), nxt*nynpy, MPI_RPREC, up,   2,   &
                  dvdt(1, 1, 0),      nxt*nynpy, MPI_RPREC, down, 2,   &
                  comm, status, ierr)

call mpi_sendrecv(dwdt(1, 1, nz - 1), nxt*nynpy, MPI_RPREC, up,   3,   &
                  dwdt(1, 1, 0),      nxt*nynpy, MPI_RPREC, down, 3,   &
                  comm, status, ierr)
!--------
call filter_data(u)
call filter_data(v)
...
if ( flag_restart .and. mod(jt, nt_restart) == 0 ) then
    call mpi_sendrecv(RHSx(1, 1, 1),  nxt*nynpy, MPI_RPREC, down, 4, &
                      RHSx(1, 1, nz), nxt*nynpy, MPI_RPREC, up,   4, &
                      comm, status, ierr)
    call mpi_sendrecv(RHSy(1, 1, 1),  nxt*nynpy, MPI_RPREC, down, 5, &
                      RHSy(1, 1, nz), nxt*nynpy, MPI_RPREC, up,   5, &
                      comm, status, ierr)
    call mpi_sendrecv(RHSz(1, 1, 1),  nxt*nynpy, MPI_RPREC, down, 6, &
                      RHSz(1, 1, nz), nxt*nynpy, MPI_RPREC, up,   6, &
                      comm, status, ierr)
    call output_final()
end if

if ( mod(nums, base_time) == 0 ) then
...
if ( inflow .and. write_inflow_file ) then
    ...
    if (PCon_flag .and. (jt .ge. PCon_init)) then		
        do jz = 1, nz-1		
            PCon(1, 1:nynpy, jz, 1) = nutrient_inflow(jz)		
        end do		
    end if
end if
