...
!! Add for 2D source read from file (Bicheng Chen 08/16/2016)

!!real(kind=rprec),dimension(:,:,:),allocatable:: scalar,q
!!real(kind=rprec),parameter::g=9.81,inv_strength=0.008 !inversion strength (K/m)
!!real(kind=rprec),dimension(nxt,nyt)::z_os !T_s and q_s are defined in bottombc.f90
!!real(kind=rprec),dimension(nxt,nyt)::ustar_avg ! Defines the local u_scale as calculated in obukhov!
!
contains
!-----------------------------------------------------------------------
!
...
!-----------------------------------------------------------------------   
    ...
    integer :: jz
    real(kind=rprec), dimension(nxt,nynpy,0:nz) :: dTdx, dTdy
    ...
    call mpi_sendrecv(w(1, 1, 1),  nxt*nynpy, MPI_RPREC, down, tag_counter+1,       &
                      w(1, 1, nz), nxt*nynpy, MPI_RPREC, up,   tag_counter+1,       &
                      comm, status, ierr)
    call mpi_sendrecv(dTdz(1, 1, 1),  nxt*nynpy, MPI_RPREC, down, tag_counter+3,    &
                      dTdz(1, 1, nz), nxt*nynpy, MPI_RPREC, up,   tag_counter+3,    &
                      comm, status, ierr)

    ! Also need to synchronize Nu_t across the processors for computation in scalar_RHS_calc (calculation of RHS_m)
    call mpi_sendrecv(Nu_t(1, 1, 1),  nxt*nynpy, MPI_RPREC, down, tag_counter+3,    &
                      Nu_t(1, 1, nz), nxt*nynpy, MPI_RPREC, up,   tag_counter+3,    &
                      comm, status, ierr)
    call mpi_sendrecv(Nu_t(1, 1, nz-1), nxt*nynpy, MPI_RPREC, up,   tag_counter+4,  &
                      Nu_t(1, 1, 0),    nxt*nynpy, MPI_RPREC, down, tag_counter+4,  &
                      comm, status, ierr)
    ...
    call mpi_sendrecv(theta(1, 1, 1),  nxt*nynpy, MPI_RPREC, down, tag_counter+7,   &
                      theta(1, 1, nz), nxt*nynpy, MPI_RPREC, up,   tag_counter+7,   &
                      comm, status, ierr)
    call mpi_sendrecv(theta(1, 1, nz-1), nxt*nynpy, MPI_RPREC, up,   tag_counter+8,     &
                      theta(1, 1, 0),    nxt*nynpy, MPI_RPREC, down, tag_counter+8,     &
                      comm, status, ierr)
    ! ---
...
!----------------------------------------------------------------------- 
    ...
    implicit none
    ! ---
    real(kind=rprec), dimension(nxt,nynpy,0:nz), intent(in) :: scalar
...
!-----------------------------------------------------------------------     
    ...
    implicit none
    ! ---
    real(kind=rprec), dimension(nxt,nynpy,0:nz) :: scalar
    real(kind=rprec), dimension(nxt,nynpy,0:nz) :: dsdx, dsdy, dsdz
    real(kind=rprec), dimension(nxt,nynpy) :: S_Surf, z_os
    real(kind=rprec), dimension(nxt,nynpy,0:nz) :: RHS
    real(kind=rprec), dimension(nxt,nynpy,0:nz) :: sgs_vert
    real(kind=rprec) :: surf_flux_current

    integer :: i, j, k, jz, jz_min, jz_max, ubc_jz
    real(kind=rprec) :: const, crap2
    real(kind=rprec), dimension(nxt,nynpy,0:nz)   :: temp, dtemp
    ...
    real(kind=rprec),dimension(nxt,nynpy) :: scalar_node_1 ! test filtered and used for computing surface flux
...
!-----------------------------------------------------------------------  
    implicit none
    ! ---
    real(kind=rprec), dimension(nxt,nynpy,0:nz) :: scalar, RHS_pre, RHS_post
    integer :: i,j,k
...
! non-MPI cases ..(with MPI in place, we can not use L_z>z_i anymore)
        if ( lz_tot > 1._rprec ) then ! for temperature and non-neutral case
...
!-----------------------------------------------------------------------
    ...
    implicit none
    ! ---
    real(kind=rprec), dimension(nxt,nynpy) :: wt_avg, theta_avg, u1, v1
...
!-----------------------------------------------------------------------
    ...
    call ddz_uv_con(dPCondz,PCon(:,:,:,1))
    ! Need to synchronize w and dPCondz across the processors for uvp node
    ! based computation in scalar_RHS_calc (calculation of RHS_m)
    call mpi_sendrecv(w(1, 1, 1),  nxt*nynpy, MPI_RPREC, down, tag_counter+1,       &
                      w(1, 1, nz), nxt*nynpy, MPI_RPREC, up,   tag_counter+1,       &
                      comm, status, ierr)
    call mpi_sendrecv(dudt(1, 1, 1),  nxt*nynpy, MPI_RPREC, down, tag_counter+2,    &
                      dudt(1, 1, nz), nxt*nynpy, MPI_RPREC, up,   tag_counter+2,    &
                      comm, status, ierr)
    call mpi_sendrecv(dvdt(1, 1, 1),  nxt*nynpy, MPI_RPREC, down, tag_counter+3,    &
                      dvdt(1, 1, nz), nxt*nynpy, MPI_RPREC, up,   tag_counter+3,    &
                      comm, status, ierr)
    call mpi_sendrecv(dwdt(1, 1, 1),  nxt*nynpy, MPI_RPREC, down, tag_counter+4,    &
                      dwdt(1, 1, nz), nxt*nynpy, MPI_RPREC, up,   tag_counter+4,    &
                      comm, status, ierr)
    ! Also need to synchronize Nu_t across the processors for computation in scalar_RHS_calc (calculation of RHS_m)
    call mpi_sendrecv(Nu_t(1, 1, 1),  nxt*nynpy, MPI_RPREC, down, tag_counter+5,    &
                      Nu_t(1, 1, nz), nxt*nynpy, MPI_RPREC, up,   tag_counter+5,    &
                      comm, status, ierr)
    call mpi_sendrecv(Nu_t(1, 1, nz-1), nxt*nynpy, MPI_RPREC, up,   tag_counter+6,  &
                      Nu_t(1, 1, 0),    nxt*nynpy, MPI_RPREC, down, tag_counter+6,  &
                      comm, status, ierr)
    call mpi_sendrecv(magS(1, 1, 1),  nxt*nynpy, MPI_RPREC, down, tag_counter+7,    &
                      magS(1, 1, nz), nxt*nynpy, MPI_RPREC, up,   tag_counter+7,    &
                      comm, status, ierr)
    call mpi_sendrecv(magS(1, 1, nz-1), nxt*nynpy, MPI_RPREC, up,   tag_counter+8,  &
                      magS(1, 1, 0),    nxt*nynpy, MPI_RPREC, down, tag_counter+8,  &
                      comm, status, ierr)
    ...
    call PCon_RHS_calc(PCon,dPCondz,PCon_sfc_temp,zo_PCon,RHS_PCon,sgs_PCon3,res_PCon3,sfc_flux,sgs_PCon1)
    
    ! For first time step, RHS(n-1)=RHS(n) for time integration
    if (((jt .eq. PCon_init) .or. (pointsource .and. (jt .eq. ini_src))) &
       .and. (.not. restart_pcon)) then
        RHS_PConf = RHS_PCon
    end if
    ...
    do ipcon = 1, npcon
        ...
        call mpi_sendrecv(PCon(:, :, 1,  ipcon), nxt*(nynpy+2), MPI_RPREC, down, tag_counter+7,     &
                          PCon(:, :, nz, ipcon), nxt*(nynpy+2), MPI_RPREC, up,   tag_counter+7,     &
                          comm, status, ierr)
        call mpi_sendrecv(PCon(:, :, nz-1, ipcon), nxt*(nynpy+2), MPI_RPREC, up,   tag_counter+8,   &
                          PCon(:, :, 0, ipcon),    nxt*(nynpy+2), MPI_RPREC, down, tag_counter+8,   &
                          comm, status, ierr)
    end do
...
!-----------------------------------------------------------------------
    ...
    if (pointsource .and. (nums >= ini_src)) then
        do ipcon = 1, npcon
    ...
    if (pointsource .and. (nums >= end_src)) THEN
        do ipcon = 1, npcon
            released=((end_src-ini_src)*dt*sum(con_src(ipcon)%rls)*dx*dy*dz)
        end do
    end if
...
!-----------------------------------------------------------------------
    ...
    ! Authored by Di Yang
    implicit none
    real(kind=rprec),dimension(nxt,0:nynpy+1,0:nz,npcon),intent(in)::PCon
    integer::i, j, k, ipcon   
...
!-----------------------------------------------------------------------
    subroutine PCon_RHS_calc(scalar,dsdz,S_Surf,z_os,RHS,sgs_vert,  &
                              res_vert,surf_flux_current,sgs_x)
!-----------------------------------------------------------------------
...
!DY Changed by Di Yang for pconsgs_acc_flag=.true.
    ...
    real(kind=rprec), dimension(nxt,0:nynpy+1,0:nz,npcon)  :: scalar
    real(kind=rprec), dimension(nxt,0:nynpy+1,0:nz,npcon)  :: dsdz
    real(kind=rprec), dimension(nxt,nynpy)             :: S_Surf,z_os
    real(kind=rprec), dimension(nxt,0:nynpy+1,0:nz,npcon)  :: RHS
    real(kind=rprec), dimension(nxt,0:nynpy+1,0:nz,npcon)  :: sgs_vert                  ! Store SGS vertical pollen flux
    real(kind=rprec), dimension(nxt,0:nynpy+1,0:nz,npcon)  :: res_vert                  ! Store resolved vertical pollen flux
    real(kind=rprec), intent(in) :: surf_flux_current  
    real(kind=rprec), dimension(nxt,0:nynpy+1,0:nz,npcon)  :: sgs_x                     ! Store SGS x-direction pollen flux

    real(kind=rprec), dimension(nxt,nynpy+1,nz)        :: u_int0,v_int0,w_int0 	      ! Interpolated velocity field without settling velocity
    ...
    allocate (u_int(nxt,nynpy+1,nz), v_int(nxt,nynpy+1,nz), w_int(nxt,nynpy+1,nz))
    allocate (ghost_x0(nynpy,nz-1), ghost_xLx(nynpy,nz-1))
    allocate (ghost_y0(nxt,nz-1), ghost_yLy(nxt,nz-1))
    allocate (ghost_z0(nxt,nynpy))
    allocate (magS_int(nxt,nynpy,0:nz) )
    ...
    call mpi_sendrecv(magS_int(1, 1, 1),  nxt*nynpy, MPI_RPREC, down, tag_counter+5,    &
                      magS_int(1, 1, nz), nxt*nynpy, MPI_RPREC, up,   tag_counter+5,    &
                      comm, status, ierr)
    call mpi_sendrecv(magS_int(1, 1, nz-1), nxt*nynpy, MPI_RPREC, up,   tag_counter+6,  &
                      magS_int(1, 1, 0),    nxt*nynpy, MPI_RPREC, down, tag_counter+6,  &
                      comm, status, ierr)
    ...
    do ipcon = 1, npcon
        ...
        call calc_srf_dep(ipcon, scalar(:,1:nynpy,:,:), S_surf, surf_flux_current, dsdz(:,1:nynpy,:,:))      
...
!-----------------------------------------------------------------------        
    ...
    ! ---
    integer :: i, j, k, ipcon  
    real(kind=rprec), dimension(nxt,0:nynpy+1,0:nz,npcon) :: scalar, RHS_pre, RHS_post
    ! ---
    do ipcon = 1, npcon
        do k = 1, nz-1
        do j = 1, nynpy
...
!-----------------------------------------------------------------------
    ...
    implicit none
    ! ---
    real(kind=rprec), dimension(nxt,nynpy+1,nz), intent(out) :: u_int0, v_int0, w_int0 
    integer :: i, j, k
...
!-----------------------------------------------------------------------
    ...
    real(rprec) :: temp
    real(kind=rprec), dimension(nxt,nz) :: temp0, temp0m, temp1, temp1m
    ! ---
    ...
    if (PCon_acc) then
        ...
        call mpi_sendrecv(temp1m(:,:), nxt*nz, mpi_rprec, north, 1, &
                          temp0(:,:),  nxt*nz, mpi_rprec, south, 1, & 
                          comm, status, ierr)
        call mpi_sendrecv(temp1(:,:),  nxt*nz, mpi_rprec, south, 2, &
                          temp0m(:,:), nxt*nz, mpi_rprec, north, 2, & 
                          comm, status, ierr)
...
!-----------------------------------------------------------------------
    ...
    ! ---
    integer, intent(in) :: ipcon
    real(kind=rprec), dimension(nxt,nynpy,0:nz,npcon), intent(in) :: scalar
    real(kind=rprec), dimension(nxt,nynpy), intent(in) :: S_Surf
    real(kind=rprec), intent(in) :: surf_flux_current
    real(kind=rprec), dimension(nxt,nynpy,0:nz,npcon), intent(out) :: dsdz
    ! ---
...
!-----------------------------------------------------------------------
    ...
    ! ---
    integer, intent(in) :: ipcon   
    real(kind=rprec), dimension(nxt,0:nynpy+1,0:nz,npcon), intent(in) :: scalar
    ! ---
    if (periodicbcx) then
    ...
    ghost_z0(1:nxt,1:nynpy) = 0._rprec
    ! ---
...
!-----------------------------------------------------------------------
    ...
    call mpi_sendrecv(Kc_t(:, :, 1),  nxt*(nynpy+2), MPI_RPREC, down, tag_counter+9,  &
                      Kc_t(:, :, nz), nxt*(nynpy+2), MPI_RPREC, up,   tag_counter+9,  &
                      comm, status, ierr)
    call mpi_sendrecv(Kc_t(:, :, nz-1), nxt*(nynpy+2), MPI_RPREC, up,   tag_counter+10,   &
                      Kc_t(:, :, 0),    nxt*(nynpy+2), MPI_RPREC, down, tag_counter+10,   &
                      comm, status, ierr)
    ! ---
...
!-----------------------------------------------------------------------
    ...
    ! ---
    integer, intent(in) :: ipcon  
    real(kind=rprec), dimension(nxt,0:nynpy+1,0:nz,npcon), intent(inout) :: RHS 
    integer :: i, j, k
...
!-----------------------------------------------------------------------
    ...
    real(kind=rprec), dimension(nxt,0:nynpy+1,0:nz,npcon), intent(in) :: scalar
    real(kind=rprec), dimension(nxt,0:nynpy+1,0:nz,npcon), intent(inout) :: RHS 
    real(kind=rprec), dimension(nxt,0:nynpy+1,0:nz), intent(out) :: sgs_vert                  ! Store SGS vertical pollen flux
    real(kind=rprec), dimension(nxt,0:nynpy+1,0:nz), intent(out) :: sgs_x 
    integer :: i, j, k
    ...
    do i = 1, nxt
        ...
        if ( coordz == 0 ) then
            ...
            sgs_vert(i,j,1)=-(1._rprec/dz)*((Kc_t(i,j,1)+Kc_t(i,j,0))/2._rprec)*(scalar(i,j,1,ipcon)-scalar(i,j,0,ipcon))
        end if
...
!-----------------------------------------------------------------------
    ...
    case (1)    
        ...
        !call mpi_sendrecv (dwdz(1, 1, nz-1), nxt*nynpy, MPI_RPREC, up, 1,  &
        !                   dwdz(1, 1, 0), nxt*nynpy, MPI_RPREC, down, 1,   &
        !                   comm, status, ierr)
        call mpi_sendrecv(acc_mag(1, 1, 1),  nxt*nynpy, MPI_RPREC, down, 2,   &
                          acc_mag(1, 1, nz), nxt*nynpy, MPI_RPREC, up,   2,   &
                          comm, status, ierr)
        if ( coordz == npz-1 ) then
    ...
    case(2)
        ...
        !                   dwdz(1, 1, 0), nxt*nynpy, MPI_RPREC, down, 1,   &
        !                   comm, status, ierr)
        call mpi_sendrecv(dwdz(1, 1, 1),  nxt*nynpy, MPI_RPREC, down, 2,  &
                          dwdz(1, 1, nz), nxt*nynpy, MPI_RPREC, up,   2,  &
                          comm, status, ierr)
...
!-----------------------------------------------------------------------
    ...
    integer :: ind
    ! --- 
    if (pointsource .and. (jt>=ini_src .and. jt<end_src)) then
    ...
    else if (flag_1Dsrc .and. (.not. pointsource) .and. (jt>=ini_src .and. jt<end_src)) then 
        do ind = 1, con_src(ipcon)%n
...
!-----------------------------------------------------------------------
    ...
    ! ---
    integer :: i, j, k
    real(kind=rprec), dimension(nxt,0:nz) :: temp1, temp2, temp3, temp4
    ! ---
    ...
    if (periodicbcy) then
        ...
        call mpi_sendrecv(temp3(:,:), nxt*(nz+1), mpi_rprec, north, 7,  &
                          temp1(:,:), nxt*(nz+1), mpi_rprec, south, 7,  &
                          comm, status, ierr)
        call mpi_sendrecv(temp4(:,:), nxt*(nz+1), mpi_rprec, north, 8,  &
                          temp2(:,:), nxt*(nz+1), mpi_rprec, south, 8,  &
                          comm, status, ierr)
...
!-----------------------------------------------------------------------
    ...
    real(kind=rprec), dimension(nxt,0:nz) :: temp2, temp4
    ! --- Interpolated concentrations in x
    ...
    ! ---
    call mpi_sendrecv(scalar(:,1:nynpy,nz-2,ipcon),nxt*nynpy,MPI_RPREC,up,&
                tag_counter+4,ghost_z0(1,1),nxt*nynpy,MPI_RPREC,down,&
