...
! --- Domain Parameters
! --- Namelist variable list
namelist/domain_param/ nxt, nyt, nzt, lx_tot, ly_tot, lz_tot, z_i, meshtype
...
character(20) :: meshtype = 'uniform'  ! mesh type in the vertical direction: uniform, cosine, pbl(planetary boundary layer)  
! --- Non-namelist variable list
...
real(rprec), allocatable, dimension(:) :: fz_uvp, fz_w         ! interpolation factor
! --- Time Parameters
! --- Namelist variable list
...
integer :: izs, ize
! --- canopy parameters
namelist/canopy_param/ flag_canopy, canopy_type, canopy_conf,     &
                       use_field_scale, use_plant_scale, hc, hgap,  &
                       lad, Cd_const, vel_exp 
logical :: flag_canopy = .true.
integer :: canopy_type = 2          ! canopy catogories, 1:submerged/emergent, 2:suspended
character(20) :: canopy_conf = 'block'  ! canopy configuration, "block" and "stripe"
logical :: use_field_canopy = .false.
logical :: use_field_scale = .true.
logical :: use_plant_scale = .false.
real(rprec) :: hc = float_missing
real(rprec) :: hgap = float_missing     ! gap between the kelp interface and the ocean surface
real(rprec) :: lad = float_missing  ! leaf area density
...
! --- Immersed Boundary Parameter
! --- Use signed-distance function to represent immersed boundary. Need to take the domain height to be unit 1.
! --- Namelist variable list
namelist /ibm_param/ flag_ibm, ratio_phic2dz, ratio_dphi, z0_ibm
logical     :: flag_ibm = .false.         ! flag to use immersed boundary method
real(rprec) :: z0_ibm = 0.1
real(rprec) :: ratio_phic2dz = 1.1_rprec  ! ratio of critical phi to dz, definition of phic is in non-namelist variables
real(rprec) :: ratio_dphi = 1.1_rprec     ! ratio of phi to dz
! --- Non-namelist variable list
character(*), parameter :: fn_ibm = './input/topography.in'
real(rprec) :: phic = float_missing    ! critical phi which defines near area of immersed boundary to use wall function to calculate the stress
real(rprec) :: dphi = float_missing    ! the distance away from immersed boundary to do the velocity interpolation for stress
...
namelist/con_param/ pcon_flag, restart_pcon, n_con, pcon_scale, &
                    ...
                    pcon_sfc, sfc_flux, models, sc, sc_boun, z_ref, settling, pcon_acc, &
                    inflow_pcon, pointsource, single_point, flag_1Dsrc, flag_2Dsrc, type_2Dsrc, tt_2Dsrc, &
                    ini_src, end_src, active_pcon, pconsgs_acc_flag, model_psgsacc, cs_pa,  &
...
real(rprec) :: z_ref = 100._rprec
logical :: settling = .false.
logical :: pcon_acc = .false.
logical :: inflow_pcon = .false.
logical :: pointsource = .true.             ! flag to set point source
logical :: single_point = .true.            ! always true anyway...
logical :: flag_1Dsrc = .false.             ! flag to use line source
logical :: flag_2Dsrc = .false.             ! flag to use 2D area source 
character(10) :: type_2Dsrc = 'scalar'      ! not used yet
integer :: tt_2Dsrc = int_missing           ! the number of time step to update the 2D source
...
! --- Particle Parameters
! --- Namelist variable list
namelist/particle_param/ vel_settling, ratio_dens, vol_spec
