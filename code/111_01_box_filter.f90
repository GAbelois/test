...
! Modules
  ...
  USE param, only:nxt, nynpy, nz

! No implicit variables
  IMPLICIT NONE

! Main variables
  REAL(kind=rprec), INTENT(in)   :: f_c(nxt, nynpy + 1, nz) ! Original field
  REAL(kind=rprec), INTENT(inout):: f_3Dh(nxt, nynpy + 1, nz) ! 3D filtered field
...
! Main variables
  REAL(kind=rprec), INTENT(in)   :: f_c(nxt, nynpy + 1, nz) ! Original field
  REAL(kind=rprec), INTENT(inout):: f_2Dxy(nxt, nynpy + 1, nz) ! 2D filtered field
  INTEGER                        :: fx, fy, fz ! Valid length of original field
