...
real(kind=rprec), dimension(nxt, nynpy, nz) :: S11, S12, S22, S33, S13, S23
!real(kind=rprec),dimension(nxt,nynpy,nz):: dissip
real(kind=rprec) :: ux, uy, uz, vx, vy, vz, wx, wy, wz
real(kind=rprec), dimension(nxt, nynpy) :: txzp, tyzp, S
...
! call mpi_sendrecv(Nu_t(1, 1, 1),  nxt*nynpy, MPI_RPREC, down, 1,  &
!                   Nu_t(1, 1, nz), nxt*nynpy, MPI_RPREC, up,   1,  &
!                   comm, status, ierr)
! call mpi_sendrecv(dissip(1, 1, 1),  nxt*nynpy, MPI_RPREC, down, 2,  &
!                   dissip(1, 1, nz), nxt*nynpy, MPI_RPREC, up,   2,  &
!                   comm, status, ierr)
! call mpi_sendrecv(magS(1, 1, 1),  nxt*nynpy, MPI_RPREC, down, 3,  &
!                   magS(1, 1, nz), nxt*nynpy, MPI_RPREC, up,   3,  &
!                   comm, status, ierr)
! ---
if ( coordz == 0 ) then
    ! bottom
...
call mpi_sendrecv(txz(1, 1, 1),  nxt*nynpy, MPI_RPREC, down, 3,    &
                  txz(1, 1, nz), nxt*nynpy, MPI_RPREC, up,   3,    &
                  comm, status, ierr)
call mpi_sendrecv(tyz(1, 1, 1),  nxt*nynpy, MPI_RPREC, down, 4,    &
                  tyz(1, 1, nz), nxt*nynpy, MPI_RPREC, up,   4,    &
                  comm, status, ierr)
