...
!--------------------------------------------------------------------!  
use types, only: rprec
use param, only: nxt, nyt, nynpy, nxhnpy
use fft
implicit none
! --- 
real(kind=rprec), dimension(nxt, nynpy), intent(inout) :: f
...
!--------------------------------------------------------------------!
use types, only: rprec
use param, only: nxt, nyt, nynpy, nxhnpy, nz, coordy, npy, comm, ierr
use fft
implicit none
! ---
real(rprec), dimension(nxt, nynpy, 0:nz), intent(inout) :: f_c
! ---
complex(kind=rprec), dimension(nxhnpy, nyt) :: tmp
real(rprec) :: const
integer :: jz
...
do jz = 0, nz
    ...
    !if ( mod(nxt, 2) == 0 .and. coordy == npy - 1 ) then
    !!if ( mod(nxt, 2) == 0 .and. coordy == 0 ) then
    !    tmp(nxhnpy, :) = 0._rprec
    !end if
    !if ( mod(nyt, 2) == 0 ) then
    !    tmp(:, nyt/2+1) = 0._rprec
    !end if
