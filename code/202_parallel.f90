...
implicit none
! ---
real(kind=rprec), dimension(nxt, 0:nynpy+1, 0:nz), intent(in)  :: a_local
real(kind=rprec), dimension(nxt, nyt, nzt),  intent(out) :: a_global
! ---
real(kind=rprec), dimension(:, :, :, :), allocatable :: temp
integer :: ip, ipy, ipz, jx, jy, jz
! ---
allocate(temp(nxt, 0:nynpy+1, 0:nz, 0:np-1))
call mpi_gather(a_local, nxt*(nynpy+2)*(nz+1), mpi_rprec,   &
                temp,    nxt*(nynpy+2)*(nz+1), mpi_rprec,   &
                0, comm, ierr)
...
implicit none
! ---
real(kind=rprec), dimension(nxt, nyt, nzt),  intent(in)  :: a_global
real(kind=rprec), dimension(nxt, nynpy, 0:nz), intent(out) :: a_local
! ---
real(kind=rprec), dimension(:, :, :, :), allocatable :: temp
integer :: ip, ipy, ipz, jx, jy, jz
! ---
...
call mpi_scatter(temp,    nxt*nynpy*(nz+1), mpi_rprec,   &
                 a_local, nxt*nynpy*(nz+1), mpi_rprec,   &
                 0, comm, ierr)
...
! ---
real(kind=rprec), dimension(nxt, nyt, nzt),  intent(in)  :: a_global
real(kind=rprec), dimension(nxt, nynpy, nz), intent(out) :: a_local
! ---
real(kind=rprec), dimension(:, :, :, :), allocatable :: temp
integer :: ip, ipy, ipz, jx, jy, jz
! ---
...
call mpi_scatter(temp,    nxt*nynpy*nz, mpi_rprec,   &
                 a_local, nxt*nynpy*nz, mpi_rprec,   &
                 0, comm, ierr)
...
implicit none
! ---
real, dimension(nxt, nyt, nzt),  intent(in)  :: a_global
real(kind=rprec), dimension(nxt, nynpy, 0:nz), intent(out) :: a_local
! ---
real(kind=rprec), dimension(:, :, :, :), allocatable :: temp
integer :: ip, ipy, ipz, jx, jy, jz
...
! ---
real(kind=rprec), dimension(nxt, nyt, nzt),  intent(in)  :: a_global
real(kind=rprec), dimension(nxt, 0:nynpy+1, 0:nz), intent(out) :: a_local
! ---
real(kind=rprec), dimension(:, :, :, :), allocatable :: temp
...
! ---
allocate(temp(nxt, 0:nynpy+1, 0:nz, 0:np-1))
allocate(temp_xz0(nxt, 0:nz), temp_xz1(nxt, 0:nz), temp_xz2(nxt, 0:nz), temp_xz3(nxt, 0:nz))
temp = 0._rprec
temp_xz0 = 0._rprec
temp_xz1 = 0._rprec
temp_xz2 = 0._rprec
...
call mpi_scatter(temp,    nxt*(nynpy+2)*(nz+1), mpi_rprec,   &
                 a_local, nxt*(nynpy+2)*(nz+1), mpi_rprec,   &
                 0, comm, ierr)

temp_xz0(:, :) = a_local(:, 1, :)
call mpi_sendrecv(temp_xz0(1, 1), nxt*(nz+1), MPI_RPREC, south, 1,       &
                  temp_xz1(1, 1), nxt*(nz+1), MPI_RPREC, north, 1,       &
                  comm, status, ierr)
a_local(:,nynpy+1,:) = temp_xz1(:, :)


temp_xz2(:, :) = a_local(:, nynpy, :)
call mpi_sendrecv(temp_xz2(1, 1), nxt*(nz+1), MPI_RPREC, south, 2,       &
                  temp_xz3(1, 1), nxt*(nz+1), MPI_RPREC, north, 2,       &
                  comm, status, ierr)
a_local(:,0,:) = temp_xz3(:, :)                  
...
call mpi_sendrecv(u(1, 1, nz - 1), nxt*nynpy, MPI_RPREC, up,   1,  &
                  u(1, 1, 0),      nxt*nynpy, MPI_RPREC, down, 1,  &
                  comm, status, ierr)
call mpi_sendrecv(v(1, 1, nz - 1), nxt*nynpy, MPI_RPREC, up,   2,  &
                  v(1, 1, 0),      nxt*nynpy, MPI_RPREC, down, 2,  &
                  comm, status, ierr)
call mpi_sendrecv(w(1, 1, nz - 1), nxt*nynpy, MPI_RPREC, up,   3,  &
                  w(1, 1, 0),      nxt*nynpy, MPI_RPREC, down, 3,  &
                  comm, status, ierr)
call mpi_sendrecv(u(1, 1, 1),  nxt*nynpy, MPI_RPREC, down, 4,  &
                  u(1, 1, nz), nxt*nynpy, MPI_RPREC, up,   4,  &
                  comm, status, ierr)
call mpi_sendrecv(v(1, 1, 1),  nxt*nynpy, MPI_RPREC, down, 5,  &
                  v(1, 1, nz), nxt*nynpy, MPI_RPREC, up,   5,  &
                  comm, status, ierr)
call mpi_sendrecv(w(1, 1, 1),  nxt*nynpy, MPI_RPREC, down, 6,  &
                  w(1, 1, nz), nxt*nynpy, MPI_RPREC, up,   6,  &
                  comm, status, ierr)
if (theta_flag) then
    call mpi_sendrecv(theta(1, 1, nz - 1), nxt*nynpy, MPI_RPREC, up,   7,  &
                      theta(1, 1, 0),      nxt*nynpy, MPI_RPREC, down, 7,  &
                      comm, status, ierr)
    call mpi_sendrecv(theta(1, 1, 1),  nxt*nynpy, MPI_RPREC, down, 8,  &
                      theta(1, 1, nz), nxt*nynpy, MPI_RPREC, up,   8,  &
                      comm, status, ierr)
end if
...
if (PCon_FLAG) then
    tag = 9
    do ipcon = 1, npcon
        call mpi_sendrecv(PCon(1, 0, nz - 1, ipcon), nxt*(nynpy+2), MPI_RPREC, up,   tag,    &
                          PCon(1, 0, 0, ipcon),      nxt*(nynpy+2), MPI_RPREC, down, tag,    &
                          comm, status, ierr)
        tag = tag + 1
    end do
end if
...
! ---
temp_xz0(:, :) = a_local(:, 1, :)
call mpi_sendrecv(temp_xz0(1, 0), nxt*(nz+1), MPI_RPREC, south, 1,       &
                  temp_xz1(1, 0), nxt*(nz+1), MPI_RPREC, north, 1,       &
                  comm, status, ierr)
a_local(:,nynpy+1,:) = temp_xz1(:, :)


temp_xz2(:, :) = a_local(:, nynpy, :)
call mpi_sendrecv(temp_xz2(1, 0), nxt*(nz+1), MPI_RPREC, north, 2,       &
                  temp_xz3(1, 0), nxt*(nz+1), MPI_RPREC, south, 2,       &
                  comm, status, ierr)
a_local(:,0,:) = temp_xz3(:, :) 

! --- synchronize the overlapping parts 0 <-> nz-1 and 1 <-> nz
call mpi_sendrecv(a_local(1, 0, nz - 1), nxt*(nynpy+2), MPI_RPREC, up,   3,    &
                  a_local(1, 0, 0),      nxt*(nynpy+2), MPI_RPREC, down, 3,    &
                  comm, status, ierr)
call mpi_sendrecv(a_local(1, 0, 1),      nxt*(nynpy+2), MPI_RPREC, down, 4,    &
                  a_local(1, 0, nz),     nxt*(nynpy+2), MPI_RPREC, up,   4,    &
                  comm, status, ierr)
! ---
...
implicit none
! ---
real(kind=rprec), dimension(nxt, 0:nynpy+1, nz), intent(out) :: a_local
logical, intent(in) :: y_per 
! ---
real(kind=rprec), dimension(nxt,nz) :: temp0, temp1, temp0m, temp1m
integer :: sou, nor
! ---
temp0  = 0._rprec
temp1  = 0._rprec
...
call mpi_sendrecv(temp1(:,:),  nxt*nz, MPI_RPREC, sou,  1,  &
                  temp0m(:,:), nxt*nz, MPI_RPREC, nor,  1,  &
                  comm, status, ierr)
call mpi_sendrecv(temp1m(:,:), nxt*nz, MPI_RPREC, nor,  2,  &
                  temp0(:,:),  nxt*nz, MPI_RPREC, sou,  2,  &
                  comm, status, ierr)
! --- periodic condition
if ( y_per ) then
    if ( idy(rank) == 0 ) then
...
!-----------------------------------------------------------------------     
use types, only: rprec
use param, only: nynpy, nz, nxt, nyt, comm_ver, mpi_rprec, ierr
use mpi
implicit none
! ---
real(rprec), dimension(nxt, nynpy, 0:nz), intent(in)  :: in
...
implicit none
! ---
real(rprec), dimension(nxt, nynpy, nz), intent(in)  :: in
real(rprec), dimension(nz-1), intent(out) :: out
! ---
