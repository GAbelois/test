...
use param, only: nxt, nynpy, nz, BOGUS, coordz, npz
use derivatives, only: ddx, ddy, ddz_w
implicit none
! ---
real(kind=rprec), dimension(nxt, nynpy, 0:nz), intent(out) :: divt
real(rprec), dimension(nxt, nynpy, 0:nz), intent(in) :: tx, ty, tz
! sc: we shouldx be able to save some memory here!
! do this by writing a divergence subroutine--then do not store derivs
real(kind=rprec), dimension(nxt, nynpy, 0:nz) :: dtxdx, dtydy, dtzdz
...
!----------------------------------------------------------------------- 
use types, only: rprec
use param, only: nxt, nynpy, nz, npz, coordz, BOGUS
use derivatives, only: ddx, ddy, ddz_uv
implicit none
! ---
real(kind=rprec), dimension(nxt, nynpy, 0:nz), intent(out) :: divt
real(kind=rprec), dimension(nxt, nynpy, 0:nz), intent(in) :: tx, ty, tz
real(kind=rprec), dimension(nxt, nynpy, 0:nz) :: dtxdx, dtydy, dtzdz
