...
do jz = 1, nz-1
    ...
    do jx = 1, nxt
        ...
        ! if (coordz == npz-1 .and. (jz == nz-1 .or. jz == nz-2)) then  ! For BBL flow
        !     noise = rms/.289_rprec*(ran3(seed) - .5_rprec)
...
! --- read initial profile to minimize inertial oscillation
allocate(nutrient_inflow(nz-1))
inquire(file='../readin/nutrient.dat', exist=exst)
if (exst) then
    open(9, file='../readin/nutrient.dat')
    read(9,*) nz_pcon
...
do jh = 1, nz_pcon-1
    if ( zuvp(jz)*z_i .ge. -hgt_PCon(jh) .and. zuvp(jz)*z_i .le. -hgt_PCon(jh+1) ) then
        ...
        c1 = 1._rprec - c0
        nutrient_inflow(jz) = (c0*PCon_inflow(jh) + c1*PCon_inflow(jh+1))*1000._rprec ! unit conversion from liter-1 to m-3 
    end if
end do
end do
...
do jz = 1, nz
    ...
    if ( z .le. z_inv/z_i ) then
        ...
        ubar(jz) = ug*(1-exp(-gamma*(lz_tot-z)*z_i)*cos(gamma*(lz_tot-z)*z_i))
