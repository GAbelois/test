...
! Modules
  ...
  USE param,only:nxt,nynpy,nz,dx, dy,dz, PCon_scheme, periodicbcx, periodicbcy
...
! Main variables
  REAL(kind=rprec),DIMENSION(nxt,nynpy,nz)       :: scalar                      ! Scalar field
  REAL(kind=rprec),DIMENSION(nxt,nynpy+1,nz)     :: u_int_tmp,v_int_tmp,w_int_tmp      ! Interpolated velocity field
  REAL(kind=rprec),DIMENSION(nxt,nynpy+1,nz)     :: magSx_int,magSy_int,magSz_int! Interpolated |S|
...
! Filtered fields
  ...
  REAL(kind=rprec),DIMENSION(nxt,nynpy+1,nz)     :: u_int_f,v_int_f,w_int_f      ! Interpolated velocity field
  REAL(kind=rprec),DIMENSION(nxt+1,nynpy+1,nz)   :: dscalar_dx,dscalar_dy,dscalar_dz ! Scalar derivatives

! Other variables
  REAL(kind=rprec),DIMENSION(nxt,nynpy+1,nz)     :: temp,temp_f                  ! Temporary to filter
  INTEGER                                    :: jz                           ! Counter
...
! Modules
  USE types,only:rprec
  USE param,only:nxt,nynpy,nz,dx,p_count,jt_total, PCon_scheme,&
...
! Modules
  USE types,only:rprec
  USE param,only:nxt,nynpy,nz,dx,PCon_scheme, periodicbcx, periodicbcy
  USE scalars_module
...
! Main variables
  ...
  REAL(kind=rprec),DIMENSION(nxt,nynpy,nz)       :: KiXi,XiXi     ! Vector dot products (2*delta)
  REAL(kind=rprec),DIMENSION(nxt,nynpy,nz)       :: KiXi2,XiXi2     ! Vector dot products (4*delta)
  REAL(kind=rprec),DIMENSION(nxt,nynpy,nz)       :: beta_c       ! Beta_c
