...
real(kind=rprec), dimension(nxt, nynpy, 0:nz) :: u_tmp, v_tmp
...
do jz = 1, nz
    ...
    if ( coordz == 0 .and. jz == 1 ) then
        ...
        ! put on uvp-nodes
        u_bar(:, :) = u_tmp(:, :, 1) ! first uv-node
        v_bar(:, :) = v_tmp(:, :, 1) ! first uv-node
        ...
        u_bar(:, :) = 0.5_rprec*(u_tmp(:, :, jz) + u_tmp(:, :, jz - 1))
        v_bar(:, :) = 0.5_rprec*(v_tmp(:, :, jz) + v_tmp(:, :, jz - 1))
        w_bar(:, :) = w(:, :, jz)
    end if
    u_hat = u_bar
    v_hat = v_bar
